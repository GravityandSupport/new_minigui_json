...
template <typename T>
void __KeyWindow<T>::registerWidget(const std::string &name, KeyWindowWidget &widget){
...
template <typename T>
void __KeyWindow<T>::registerWidget(const std::string &name, size_t row, KeyWindowWidget &widget){
	BaseWindow::registerWidget(name, widget);
	// 策略：自动填充缺失的中间行（空行）
...
//}
template <typename T>
std::vector<KeyWindowWidget*>& __KeyWindow<T>::getRow(size_t row) {
    if (row >= key_layout.size()) {
...
template <typename T>
KeyWindowWidget* __KeyWindow<T>::getRow(size_t row, size_t col) const {
...
template <typename T>
bool __KeyWindow<T>::setFocus(size_t row, size_t col) {
    if (row >= key_layout.size() || col >= key_layout[row].size()) {
        return false;
    }
...
template <typename T>
bool __KeyWindow<T>::setFocus(const std::string& name) {
    auto it = this->registry_widget.find(name);
    if (it == this->registry_widget.end()) return false;
    return setFocus(it->second);
}
template <typename T>
bool __KeyWindow<T>::setFocus(Widget* widget) {
...
template <typename T>
bool __KeyWindow<T>::trySetFocus(int row, int col) {
...
template <typename T>
std::pair<int, int> __KeyWindow<T>::findWidgetPosition(Widget* widget) const {
...
template <typename T>
std::pair<int, int> __KeyWindow<T>::findWidgetPosition(const std::string& name) const {
    auto it = this->registry_widget.find(name);
    if (it == this->registry_widget.end()) return {-1, -1};
    return findWidgetPosition(it->second);
}
template <typename T>
size_t __KeyWindow<T>::getColumnCount(size_t row) const {
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
template <typename T>
bool __KeyWindow<T>::focusMoveControl(WPARAM wParam, LPARAM lParam)  {
    switch(wParam){
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
template <typename T>
bool __KeyWindow<T>::sendMessageToCurrent(void (Widget::*func)(WPARAM, LPARAM), WPARAM wParam, LPARAM lParam){
...
template <typename T>
bool __KeyWindow<T>::sendMessageToWidget(size_t row, size_t col, 
...
template <typename T>
bool __KeyWindow<T>::sendMessageByName(const std::string& name, 
    									void (Widget::*func)(WPARAM, LPARAM), WPARAM wParam, LPARAM lParam) {
	Widget* widget = this->findWidget(name);
	if (!widget) {
		return false;  // 未找到控件
	}
...
template <typename T>
void __KeyWindow<T>::sendMessageToAll(void (Widget::*func)(WPARAM, LPARAM), WPARAM wParam, LPARAM lParam) {
